  ...
  function renderWebinars() {
    ...
    list.innerHTML = filtered.map(w => {
      ...
      const attachLinks = (w.attachments && w.attachments.length > 0)
        ? w.attachments.map(a => `<a class="attach-link" onclick="openAttachment('${a.url}', '${safeTitle} - ${a.name}', event)" title="${a.name}">
      ...
      const docLink = w.docUrl
        ? `<a class="attach-link doc-link" onclick="openAttachment('${w.docUrl}', '${safeTitle} - 官方文档', event)" title="官方文档">
